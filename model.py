...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        # because the weights of the embedding layer and the linear layer are the same
        # reduces the number of parameters, as the weights are shared

        self.apply(self._init_weights)

    def _init_weights(self, module): #Xavier initialization. std is roughly the square root of the number of inputs (dimensions)
        std = 0.02
        ...
        if isinstance(module, nn.Linear):
            ...
            if module.bias is not None:
                torch.nn.init.zeros_(module.bias) #initialize the bias to zero
    ...
    def configure_optimizers(self, weight_decay, learning_rate, device_type):
        ...
        # start with all of the parameters that require gradients
        ...
        # i.e all the weight tensors in matmuls + Embeddings decay, all bias and layernorms do not decay.
        ...
        print(f"num no decayed params tensors: {len(no_decay_params)}, with {num_no_decay_params:,} parameters")

        fused_available = 'fused' in inspect.signature(torch.optim.AdamW).parameters
        use_fused = fused_available and device_type == 'cuda'
        print(f"using fused Adam optimizer with {use_fused}")
